  ...
  const api = {
    ...
    async listAllTopics(cluster) {
      ...
      while (true) {
        ...
        else if (data?.data || data?.items) items = data.data || data.items;
        else {
        ...
        if (totalPages != null) { if (page >= totalPages) break; page++; }
    ...
    async send(cluster, topic, partition, contentStr) {
      ...
      const body = { partition, key: null, content: contentStr, keySerde: 'String', valueSerde: 'String' };
  ...
  root.innerHTML = `
    <style>
      * { box-sizing: border-box; font-family: Inter, system-ui, sans-serif; }
      ...
      textarea {
        ...
        overflow:auto;              /* show horizontal scroll if needed */
        white-space: pre;           /* NO WRAP: keep JSON on one line */
      }
      .inline { display:flex; align-items:center; gap:6px; }
      .grid2 { display:grid; grid-template-columns: 1fr 200px; gap:10px; }
      .search { width:100%; margin:6px 0; }
      .count { color:#9ca3af; font-size:12px; margin-left:6px; }
      .status {
        max-height:180px; overflow:auto; border:1px solid #d1d5db;
    ...
    <div class="card">
      ...
      <div class="row">
        ...
        <textarea id="msgBox" wrap="off" spellcheck="false" placeholder='{"messageType":"REQUEST","messageId":"mid","uri":"update","data":{"s":"ABC123","is":"KAFI"}}'></textarea>
      </div>
